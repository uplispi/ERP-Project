...
export const downloadWarehouseCounting = config => request._download(testAPI.WAERHOUSE_DAILY_EXCEL, config);

export const getWarehouseGiftSheetByState = config => request._get(testAPI.WAREHOUSE_GIFT_SHOW, config);
export const approvalWarehouseGiftSheet = config => request._get(testAPI.WAREHOUSE_GIFT_APPROVAL, config);
