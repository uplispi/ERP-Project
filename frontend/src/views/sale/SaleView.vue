    ...
    >
      <div style="width: 90%; margin: 0 auto">
        ...
        >
          ...
          <el-form-item label="可用的促销方案：">
            <el-select
              v-model="saleForm.promotionId"
              placeholder="请选择促销方案"
            >
              <el-option
                v-for="item in promotionList.filter(
                  (item1) => !seller || item1.level <= seller.level
                )"
                :key="item.id"
                :value="item.id"
                :label="item.name"
...
import { getValidPromotion } from "@/network/promotion";
export default {
  name: "SaleView",
  components: {
    Layout,
  ...
  computed: {
    seller: function () {
      return this.sellers.filter((item) => {
        return item.id === this.saleForm.supplier;
      })[0];
    },
  },
  mounted() {
  ...
  methods: {
    ...
    permitted() {
      // 进货销售才可以制定销售单
      return (
        sessionStorage.getItem("role") === "SALE_STAFF" ||
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.saleForm.promotionId = Number(this.saleForm.promotionId);
          this.saleForm.discount = 10;
          this.saleForm.voucherAmount = 0;
