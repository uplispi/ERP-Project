  ...
  <div class="card">
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        <el-row>
          <el-col :span="3">
            <span><strong>库存赠品单编号: </strong>{{item.id}}</span>
          </el-col>
          <el-col :span="3">
            <span><strong>操作员: </strong>{{item.operator}}</span>
          </el-col>
          <el-col :span="3">
            <span><strong>顾客id: </strong>{{item.customerId}}</span>
          </el-col>
          <el-col :span="6">
            <span><strong>销售单id: </strong>{{item.saleSheetId}}</span>
          </el-col>
        </el-row>
...
<script>
import {approvalWarehouseGiftSheet} from "@/network/warehouse";
import {approvalCollectionSheet} from "@/network/finance";

export default {
  name: "InventoryGiftSheetList",
  props: {
    list: Array,
    type: Number,
  },
  ...
  methods: {
    ...
    approval(id) {
      let config = {
        params: {
          warehouseGiftSheetId: id,
          state: 'SUCCESS'
        }
      }
      approvalWarehouseGiftSheet(config).then(res => {
        this.$emit("refresh")
        this.$message({
          message: '操作成功!',
          type: 'success'
