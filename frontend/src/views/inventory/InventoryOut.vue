<template>
  <div>
    ...
    <el-dialog title="确认出货单" :visible.sync="dialogVisible" width="40%" :before-close="handleClose"
      ...
      <div class="basic-info">
        ...
        <template v-for='item in inventoryList'>
          <div class="type">
            <h3>{{item[0]}}</h3>
            <div>{{ inventoryOutConfirmList[0][item[1]] }}</div>
          </div>
          <!-- <div class="type">
            <h3>批次</h3>
...
export default {
  ...
  data() {
    return {
      inventoryList : [['批次', 'batchId'], ['购价', 'purchasePrice'], ['选购数量', 'selectedQuantity'], ['批次总量', 'totalQuantity'], ['总金额', 'sumPrice'], ['备注', 'remark']],
      card_type: "出库",
