<template>
  <Layout>
    ...
    <el-button v-if="permitted()" type="primary" size="medium" @click="dialogVisible = true">制定进货单</el-button>
    <div class="body">
...
export default {
  ...
  methods: {
    permitted(){
      // 进货销售才可以制定进货单
      return sessionStorage.getItem('role') === 'SALE_STAFF'|| sessionStorage.getItem('role') ==='SALE_MANAGER' || sessionStorage.getItem('role') === 'ADMIN'
    },
    getPurchase() {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          for(let item of this.purchaseForm.purchaseSheetContent){
            console.log('item=>', item)
            item.id = null
            item.purchaseSheetId = null
            if(this.validateNumber(item.quantity) && this.validateNumber(item.unitPrice)){
              item.quantity = parseInt(item.quantity)
              item.unitPrice = parseInt(item.unitPrice)
            }
            else{
              this.$message.error('数量或价格必须是正整数')
              return
            }
